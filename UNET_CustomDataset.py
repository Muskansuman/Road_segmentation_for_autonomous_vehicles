"""UNet semantic segmentation training pipeline for custom road datasets."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train UNet for road segmentation")
    parser.add_argument(
        "--img-path",
        type=Path,
        default=Path("data/training/images"),
        help="Directory containing training images",
    )
    parser.add_argument(
        "--mask-path",
        type=Path,
        default=Path("data/training/masks"),
...
def loadImages(path,mask=False):
    ...
    for image in images:
        ...
        resized_im = cv2.resize(im, (IMAGE_SIZE, IMAGE_SIZE))
